...
try {
  ...
  if (configStr) {
    ...
    auth = getAuth(app);
    db = getFirestore(app);
    // 修复：平台自动生成的 __app_id 可能包含 "/"，导致 Firestore 集合路径段数错乱。这里将 "/" 替换为 "_"
    rootAppId = (typeof __app_id !== 'undefined' ? String(__app_id) : 'default-app-id').replace(/\//g, '_');
  } 
  // 兼容您自己本地/Vercel 部署的环境，改用更安全的读取方式
  else {
    let fbConfig = null;
    if (typeof process !== 'undefined' && process.env && process.env.VITE_FB_API_KEY) {
      fbConfig = {
      ...
      rootAppId = String(process.env.VITE_FB_APP_ID || 'custom-app').replace(/\//g, '_');
    } else if (typeof window !== 'undefined' && window.VITE_FB_API_KEY) {
      fbConfig = {
        apiKey: window.VITE_FB_API_KEY,
        authDomain: window.VITE_FB_AUTH_DOMAIN,
        projectId: window.VITE_FB_PROJECT_ID,
        storageBucket: window.VITE_FB_STORAGE_BUCKET,
        messagingSenderId: window.VITE_FB_MESSAGING_SENDER_ID,
        appId: window.VITE_FB_APP_ID,
        measurementId: window.VITE_FB_MEASUREMENT_ID
      };
      rootAppId = String(window.VITE_FB_APP_ID || 'custom-app').replace(/\//g, '_');
    }
...
export default function App() {
    ...
    const initAuth = async () => {
      try {
        if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
          ...
          await signInAnonymously(auth); // 若无指定token，则分配匿名云端账号
        }
      } catch(e) {
  ...
  useEffect(() => {
    ...
    try {
      // 定位到当前用户的专属单词集合（修复后 rootAppId 无非法字符，确保路径段数为奇数）
  ...
  const handleAddWord = async (e) => {
    ...
    try {
      ...
      setErrorMsg(err.message ? String(err.message) : "未知错误。");
    } finally {
      setIsAdding(false);
    }
  };
  ...
  const filteredWords = words
    ...
    .filter(w => {
      const safeWord = String(w.word || '');
      const safeTranslation = String(w.translation || '');
        ...
        {errorMsg && (
          <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl flex items-start justify-between shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <p className="text-red-700 text-sm font-medium">{String(errorMsg)}</p>
            </div>
                    ...
                    <tr key={word.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1 max-w-lg">
                          ...
                          <span className="text-xs text-gray-500 whitespace-normal mt-1">
                            {String(word.exampleTranslation)}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        <div className="flex items-center justify-center gap-3">
                          ...
                          >
                            {word.memorized ? (
                              <span className="flex items-center gap-1.5"><CheckCircle size={16} /> 已掌握</span>
                            ) : (
                              <span className="flex items-center gap-1.5"><Circle size={16} /> 学习中</span>
                            )}
                          </button>
